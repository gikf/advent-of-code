...
def main(file_input='input.txt'):
    ...
    program, state = process_program(intcodes[:], get_state())
    grid = create_grid(state['output'])
    intersections = find_intersections(grid)
    alignment = sum(row * col for row, col in intersections)
    print(f'Alignment: {alignment}')
    collected_dust = collect_dust(intcodes[:])
    print(f'Collected dust: {collected_dust}')


def collect_dust(program):
    """Run vacuum robot to collect dust."""
    program[0] = 2
    commands = [
        'A,B,B,A,C,A,A,C,B,C',
        'R,8,L,12,R,8',
    ...
    for command in commands:
        state['input'].extend(parse_input(command))
    program, state = process_program(program, state)
    return state['output'][-1]


def parse_input(command):
    """Parse command to input format."""
    return [ord(char) for char in command] + [10]


def find_intersections(grid):
    """Find intersections on the grid."""
    intersections = []
    for row_no, row in enumerate(grid[1:-1], start=1):
        for col_no, col in enumerate(row[1:-1], start=1):
            if col != '#':
                continue
            adjacent = get_adjacent(row_no, col_no, grid)
            if all(cell == '#' for cell in adjacent):
                intersections.append((row_no, col_no))
    return intersections


def get_adjacent(row_no, col_no, grid):
    """Get adjacent cells on grid to (row_no, col_no)."""
    adjacent = []
    for move in MOVES:
        adjacent_y, adjacent_x = [
            value + change
            for value, change in zip((row_no, col_no), move)]
        adjacent.append(grid[adjacent_y][adjacent_x])
    return adjacent
