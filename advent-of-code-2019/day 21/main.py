...
def main(file_input='input.txt'):
    ...
    for description, instructions in runs:
        state = get_state()
        state['input'] = prepare_input(instructions)
        damage = find_hull_damage(intcodes[:], state)
        print(f'Hull damage found with {description} command: {damage}')


def find_hull_damage(program, state):
    """Find hull damage using program and state with input."""
    program, state = process_program(program, state)
    return state['output'][-1]


def prepare_input(instructions):
    """Prepare instructions for the droid."""
    input_value = []
    for instruction in instructions:
        input_value.extend(parse_input(instruction))
    return input_value
